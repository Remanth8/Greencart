...
const Navbar = () => {
  ...
  return (
        ...
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white sticky top-0 z-50">
            ...
            <div className="hidden sm:flex items-center gap-10 font-medium text-gray-700">
               ...
               {!user ? ( <button onClick={()=> setShowUserLogin(true)} className="cursor-pointer
                px-8 py-2 bg-primary hover:bg-primary-dull transition text-white rounded-full">
                    Login
                </button> )
                :
                (   
...
{/* Mobile Menu */}
{open && (
  <div className="fixed inset-0 bg-white z-50 flex flex-col px-6 py-8 md:hidden">
    ...
    {!user ? (
      <button
        ...
        className="mt-6 bg-primary text-white py-3 rounded-full"
      >
        Login
      </button>
    ) : (
